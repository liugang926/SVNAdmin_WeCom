...
- LDAP 用户同步会记录被跳过的非法用户名；LDAP 分组同步会检测与手工分组同名的冲突。
- LDAP 分组发现同名冲突时会跳过冲突 LDAP 分组、继续同步其它分组，并在完成后提示冲突分组清单，避免 LDAP 成员和手工成员被静默合并。
- 用户/分组同步失败或部分跳过会写入系统日志中的 `LDAP同步审计`，管理员可在系统日志页点击“同步审计”快速筛选处理清单。
...
如果 LDAP 分组名与手工分组名重复，冲突的 LDAP 分组会被跳过，原有手工分组保持不变；其它 LDAP 分组会继续同步。建议优先采用统一命名策略，例如给本地手工分组加 `local-` 前缀，或在 LDAP 分组字段映射中使用不会与本地分组冲突的属性。处理完冲突后再重新执行分组同步。
...
这表示 LDAP 返回的分组名与 SVNAdmin 手工分组名相同。系统会跳过冲突的 LDAP 分组并继续同步其它分组，避免两个来源的成员被写入同一个 authz 分组。请根据提示清单重命名手工分组，或调整 LDAP 分组名称映射/固定前缀后重新同步。

### LDAP 同步审计中存在失败或跳过项

系统日志页的“同步审计”会筛选 `LDAP同步审计` 记录。常见原因包括 LDAP 连接失败、搜索过滤器或字段映射错误、用户名不符合 SVN 用户名规则、LDAP 分组与手工分组同名。请根据审计记录中的对象清单和处理建议修正 LDAP 映射或本地分组命名后重新同步。
