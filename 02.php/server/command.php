...
class Command
{
     ...
     * @return void
     */
    public function SyncWeComUsers()
    {
        print_r(sprintf('开始执行企业微信用户同步任务%s', PHP_EOL));
        
        try {
            require_once BASE_PATH . '/app/service/wecom/WeComUserSync.php';
            $userSync = new \app\service\wecom\WeComUserSync();
            $result = $userSync->syncUsers();
            
            if ($result['status'] === 1) {
                print_r(sprintf('企业微信用户同步成功: %s%s', $result['message'], PHP_EOL));
                $this->code = 0;
            } else {
                print_r(sprintf('企业微信用户同步失败: %s%s', $result['message'], PHP_EOL));
                $this->code = 1;
            }
        } catch (\Exception $e) {
            print_r(sprintf('企业微信用户同步异常: %s%s', $e->getMessage(), PHP_EOL));
            $this->code = 1;
        }
     ...
     * @return void
     */
    public function SyncWeComDepartments()
    {
        print_r(sprintf('开始执行企业微信分组同步任务%s', PHP_EOL));
        
        try {
            require_once BASE_PATH . '/app/service/wecom/WeComGroupSync.php';
            $groupSync = new \app\service\wecom\WeComGroupSync();
            $result = $groupSync->syncDepartments();
            
            if ($result['status'] === 1) {
                print_r(sprintf('企业微信分组同步成功: %s%s', $result['message'], PHP_EOL));
                $this->code = 0;
            } else {
                print_r(sprintf('企业微信分组同步失败: %s%s', $result['message'], PHP_EOL));
                $this->code = 1;
            }
        } catch (\Exception $e) {
            print_r(sprintf('企业微信分组同步异常: %s%s', $e->getMessage(), PHP_EOL));
            $this->code = 1;
        }
     ...
     * @return void
     */
    public function SyncWeComRepositories()
    {
        print_r(sprintf('开始执行企业微信仓库权限同步任务%s', PHP_EOL));
        
        try {
            require_once BASE_PATH . '/app/service/wecom/WeComPermissionSync.php';
            $permissionSync = new \app\service\wecom\WeComPermissionSync();
            $result = $permissionSync->syncRepositoryPermissions();
            
            if ($result['status'] === 1) {
                print_r(sprintf('企业微信仓库权限同步成功: %s%s', $result['message'], PHP_EOL));
                $this->code = 0;
            } else {
                print_r(sprintf('企业微信仓库权限同步失败: %s%s', $result['message'], PHP_EOL));
                $this->code = 1;
            }
        } catch (\Exception $e) {
            print_r(sprintf('企业微信仓库权限同步异常: %s%s', $e->getMessage(), PHP_EOL));
            $this->code = 1;
        }
     ...
     * @return void
     */
    public function WeComFullSync()
    {
        print_r(sprintf('开始执行企业微信全量同步任务%s', PHP_EOL));
        
        try {
            require_once BASE_PATH . '/app/service/WeComSync.php';
            $wecomSync = new \app\service\WeComSync();
            $result = $wecomSync->fullSync();
            
            if ($result['status'] === 1) {
                print_r(sprintf('企业微信全量同步成功: %s%s', $result['message'], PHP_EOL));
                $this->code = 0;
            } else {
                print_r(sprintf('企业微信全量同步失败: %s%s', $result['message'], PHP_EOL));
                $this->code = 1;
            }
        } catch (\Exception $e) {
            print_r(sprintf('企业微信全量同步异常: %s%s', $e->getMessage(), PHP_EOL));
            $this->code = 1;
        }
     ...
     * @return void
     */
    public function WeComIncrementalSync()
    {
        print_r(sprintf('开始执行企业微信增量同步任务%s', PHP_EOL));
        
        try {
            require_once BASE_PATH . '/app/service/WeComSync.php';
            $wecomSync = new \app\service\WeComSync();
            $result = $wecomSync->incrementalSync();
            
            if ($result['status'] === 1) {
                print_r(sprintf('企业微信增量同步成功: %s%s', $result['message'], PHP_EOL));
                $this->code = 0;
            } else {
                print_r(sprintf('企业微信增量同步失败: %s%s', $result['message'], PHP_EOL));
                $this->code = 1;
            }
        } catch (\Exception $e) {
            print_r(sprintf('企业微信增量同步异常: %s%s', $e->getMessage(), PHP_EOL));
            $this->code = 1;
        }
...
switch ($argv[1]) {
    ...
    case 9: //同步SVN仓库
        ...
        break;
    case 10: //同步企业微信用户
        $command->SyncWeComUsers();
        break;
    case 11: //同步企业微信分组
        $command->SyncWeComDepartments();
        break;
    case 12: //同步企业微信仓库
        $command->SyncWeComRepositories();
        break;
    case 13: //企业微信全量同步
        $command->WeComFullSync();
        break;
    case 14: //企业微信增量同步
        $command->WeComIncrementalSync();
        break;
    default:
        break;
}
