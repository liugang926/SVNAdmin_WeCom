...
class Daemon
{
    ...
    private $configSvn;
    private $configWecom;
    private $wecomSyncPidFile;
    private $lastWecomSyncTime;
    private $lastWecomCleanupTime;
    private $lastWecomNotificationTime;
    private $wecomNotificationHandler;

    function __construct()
    {
        $this->masterPidFile = dirname(__FILE__) . '/svnadmind.pid';
        $this->taskPidFile = dirname(__FILE__) . '/task_svnadmind.pid';
        $this->wecomSyncPidFile = dirname(__FILE__) . '/wecom_sync.pid';

        Config::load(BASE_PATH . '/config/');
        $this->configDaemon = Config::get('daemon');
        $this->configSvn = Config::get('svn');
        $this->configWecom = Config::get('wecom');
        $this->lastWecomSyncTime = 0;
        $this->lastWecomCleanupTime = 0;
        $this->lastWecomNotificationTime = 0;
        
        // 初始化企业微信通知处理器
        require_once dirname(__FILE__) . '/WeComNotificationHandler.php';
        $this->wecomNotificationHandler = new WeComNotificationHandler($this->configSvn);
    }
    ...
    private function InitSocket()
    {
        ...
        // 创建企业微信同步进程
        if (isset($this->configWecom['enabled']) && $this->configWecom['enabled']) {
            $wecomPid = pcntl_fork();
            if ($wecomPid == -1) {
                die(sprintf('企业微信同步进程创建失败[%s]%s', socket_strerror(socket_last_error()), PHP_EOL));
            } elseif ($wecomPid == 0) {
                file_put_contents($this->wecomSyncPidFile, getmypid());
                
                while (true) {
                    $this->HandleWeComSync();
                    
                    // 检查是否需要执行数据清理
                    $this->checkAndCleanupWeComData();
                    
                    sleep($this->configWecom['sync_interval'] ?? 300); // 默认5分钟同步一次
                }
                exit();
            }
        }
    ...
    {
        // 处理企业微信通知（每5秒检查一次）
        if (time() - $this->lastWecomNotificationTime >= 5) {
            $this->wecomNotificationHandler->process();
            $this->lastWecomNotificationTime = time();
        }
    ...
    private function Stop()
    {
        ...
        if (file_exists($this->wecomSyncPidFile)) {
            $pid = file_get_contents($this->wecomSyncPidFile);
            posix_kill((int)$pid, 9);
            @unlink($this->wecomSyncPidFile);
        }
     ...
     * 处理企业微信同步任务
     */
    private function HandleWeComSync()
    {
        $wecom_error_log_file = $this->configSvn['log_base_path'] . 'wecom_sync_error.log';
        ...
        try {
            ...
            // 检查同步间隔
            $currentTime = time();
            $syncInterval = $this->configWecom['sync_interval'] ?? 300;
            
            if ($currentTime - $this->lastWecomSyncTime < $syncInterval) {
                return;
            }

            $this->lastWecomSyncTime = $currentTime;

            // 记录同步开始
            $message = sprintf('[%s][企业微信同步开始]%s', date('Y-m-d H:i:s'), PHP_EOL);
            file_put_contents($wecom_error_log_file, $message, FILE_APPEND);
            ...
            // 初始化企业微信同步服务
            $wecomSync = new WeComSync();
            
            // 执行同步任务 - 使用重构后的全量同步方法
            try {
                $syncResult = $wecomSync->fullSync();
                
                if ($syncResult['status'] === 1) {
                    $message = sprintf('[%s][企业微信同步完成][%s]%s', 
                        date('Y-m-d H:i:s'), 
                    ...
                    $message = sprintf('[%s][企业微信同步失败][%s]%s', 
                        date('Y-m-d H:i:s'), 
                ...
                $message = sprintf('[%s][企业微信同步异常][%s]%s', date('Y-m-d H:i:s'), $e->getMessage(), PHP_EOL);
                file_put_contents($wecom_error_log_file, $message, FILE_APPEND);
            }

            // 记录同步完成
            $message = sprintf('[%s][企业微信同步完成]%s', date('Y-m-d H:i:s'), PHP_EOL);
            file_put_contents($wecom_error_log_file, $message, FILE_APPEND);

        } catch (\Exception $e) {
            $message = sprintf('[%s][企业微信同步守护进程异常][%s]%s', date('Y-m-d H:i:s'), $e->getMessage(), PHP_EOL);
            file_put_contents($wecom_error_log_file, $message, FILE_APPEND);
        }
    }
     ...
     * 记录企业微信同步日志
     */
    private function recordWeComSyncLog($database, $syncResults)
    {
        try {
            $logData = [
                'sync_type' => 'daemon_auto',
                'sync_status' => 'completed',
                'departments_synced' => $syncResults['departments']['success_count'] ?? 0,
                'users_synced' => $syncResults['users']['success_count'] ?? 0,
                'permissions_synced' => $syncResults['permissions']['success_count'] ?? 0,
                'error_count' => ($syncResults['departments']['error_count'] ?? 0) + 
                               ($syncResults['users']['error_count'] ?? 0) + 
                               ($syncResults['permissions']['error_count'] ?? 0),
                'sync_details' => json_encode($syncResults),
                'created_at' => date('Y-m-d H:i:s')
            ];

            $database->insert('wecom_sync_logs', $logData);
        } catch (\Exception $e) {
            $wecom_error_log_file = $this->configSvn['log_base_path'] . 'wecom_sync_error.log';
            $message = sprintf('[%s][同步日志记录失败][%s]%s', date('Y-m-d H:i:s'), $e->getMessage(), PHP_EOL);
            file_put_contents($wecom_error_log_file, $message, FILE_APPEND);
        }
    }
     ...
     * 检查并执行企业微信数据清理
     */
    private function checkAndCleanupWeComData()
    {
        $currentTime = time();
        $cleanupInterval = $this->configDaemon['wecom_sync']['cleanup_interval'] ?? 86400; // 默认24小时
        
        if ($currentTime - $this->lastWecomCleanupTime >= $cleanupInterval) {
            $this->lastWecomCleanupTime = $currentTime;
            $this->cleanupWeComData();
        }
    }
     ...
     * 清理企业微信相关的过期数据
     */
    private function cleanupWeComData()
    {
        $wecom_error_log_file = $this->configSvn['log_base_path'] . 'wecom_sync_error.log';
        ...
        try {
            ...
            $database->delete('wecom_sync_logs', [
                'created_at[<]' => $cleanupDate
            ]);

            // 清理30天前的通知日志
            $database->delete('wecom_notification_logs', [
                'created_at[<]' => $cleanupDate
            ]);

            $message = sprintf('[%s][企业微信数据清理完成]%s', date('Y-m-d H:i:s'), PHP_EOL);
            file_put_contents($wecom_error_log_file, $message, FILE_APPEND);

        } catch (\Exception $e) {
            $message = sprintf('[%s][企业微信数据清理异常][%s]%s', date('Y-m-d H:i:s'), $e->getMessage(), PHP_EOL);
            file_put_contents($wecom_error_log_file, $message, FILE_APPEND);
        }
    }
