...
class SVNAdmin
{
     ...
     * @return string
     */
    private function updateRepositoryReferencesInPermissions($authzContent, $oldRepName, $newRepName)
    {
        // 分割内容为行进行处理
        $lines = explode("\n", $authzContent);
        $updatedLines = [];
        ...
        foreach ($lines as $line) {
            ...
            if ($inSection && strpos($line, '=') !== false) {
                // 检查权限配置中是否包含旧仓库名的引用
                // 例如：@group = r 或者 user = rw 等情况下，可能在注释或其他地方引用仓库名
                $updatedLine = $line;
                
                // 如果当前节不是我们要重命名的仓库节，但权限配置中可能引用了仓库名
                if (strpos($currentSection, $oldRepName . ':') !== 0) {
                    // 在权限配置的注释部分替换仓库名引用
                    if (strpos($line, '#') !== false) {
                        $updatedLine = str_replace($oldRepName, $newRepName, $line);
                    }
                }
