...
class Svnrep extends Base
{
    ...
    {
        $sync = $this->payload['sync'] ?? false;
        $page = $this->payload['page'] ?? false;

        $sync_size = $this->payload['sync_size'] ?? false;
        $sync_rev = $this->payload['sync_rev'] ?? false;
        ...
        foreach ($list as $key => $value) {
            $list[$key]['rep_size'] = funFormatSize($value['rep_size'] ?? 0);
            $list[$key]['rep_rev'] = $value['rep_rev'] ?? 0;
    ...
    public function UpdRepName()
    {
        ...
        //更新数据库中相关的权限记录
        $this->updateRepositoryPermissionsInDatabase($this->payload['old_rep_name'], $this->payload['new_rep_name']);

        //从配置文件迁移仓库权限配置
        $authzUpdateResult = $this->migrateRepositoryPermissionsInAuthz($this->payload['old_rep_name'], $this->payload['new_rep_name']);
        if (!$authzUpdateResult['success']) {
            return message(200, 0, $authzUpdateResult['message']);
        }
    ...
    private function updateRepositoryPermissionsInDatabase($oldRepName, $newRepName)
    {
        try {
            ...
            // 检查表是否存在的方法需要使用不同的方式
            try {
                $this->database->select('svn_group_pri_paths', '*', ['LIMIT' => 1]);
                // 如果没有异常，表存在
            ...
            // 更新企业微信相关权限记录（如果存在）
            try {
                $this->database->select('wecom_sync_logs', '*', ['LIMIT' => 1]);
                // 如果没有异常，表存在
            ...
            $this->ServiceLogs->InsertLog(
                '更新仓库权限记录',
                sprintf("仓库重命名权限更新: %s -> %s", $oldRepName, $newRepName),
                $this->userName
            );
     ...
     * 迁移仓库权限配置（从authz文件）
     * 
     * @param string $oldRepName 原仓库名
     * @param string $newRepName 新仓库名
     * @return array 操作结果
     */
    private function migrateRepositoryPermissionsInAuthz($oldRepName, $newRepName)
    {
        try {
            // 1. 首先检查新仓库名是否已经存在权限配置（避免冲突）
            $newRepoPattern = '/\[' . preg_quote($newRepName) . ':.*?\]/';
            if (preg_match($newRepoPattern, $this->authzContent)) {
                return [
                    'success' => false,
                    'message' => "新仓库名 '{$newRepName}' 在authz中已存在权限配置，无法迁移"
                ];
            }

            // 2. 尝试迁移原仓库的权限配置
            $result = $this->SVNAdmin->UpdRepFromAuthz($this->authzContent, $oldRepName, $newRepName);
            ...
            if (is_numeric($result)) {
                if ($result == 740) {
                    ...
                    $this->ServiceLogs->InsertLog(
                        '仓库权限迁移',
                        sprintf("仓库 '%s' 重命名为 '%s'：原仓库无权限配置，无需迁移", $oldRepName, $newRepName),
                        $this->userName
                    );
                    return [
                        'success' => true,
                        'message' => '权限迁移完成（原仓库无权限配置）'
                    ];
                } elseif ($result == 751) {
                    return [
                        'success' => false,
                        'message' => '在authz配置中找不到指定仓库'
                    ];
                } else {
                    return [
                        'success' => false,
                        'message' => "authz权限迁移失败，错误码: $result"
                    ];
                }
            } else {
                ...
                $this->ServiceLogs->InsertLog(
                    '仓库权限迁移',
                    sprintf("仓库 '%s' 重命名为 '%s'：权限配置已成功迁移", $oldRepName, $newRepName),
                    $this->userName
                );
                
                return [
                    'success' => true,
                    'message' => '权限迁移完成（已更新authz配置）'
                ];
            }
            ...
            $this->ServiceLogs->InsertLog(
                '仓库权限迁移失败',
                sprintf("仓库 '%s' 重命名为 '%s' 时权限迁移异常: %s", $oldRepName, $newRepName, $e->getMessage()),
                $this->userName
            );
            
            return [
                'success' => false,
                'message' => '权限迁移过程中发生异常: ' . $e->getMessage()
            ];
        }
    }
}
