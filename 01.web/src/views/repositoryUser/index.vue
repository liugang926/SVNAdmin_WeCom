...
export default {
  ...
  methods: {
    ...
    loadColumnWidths() {
      // 从本地存储加载列宽设置
      const savedWidths = localStorage.getItem('svn_user_column_widths');
      if (savedWidths) {
        try {
          const widths = JSON.parse(savedWidths);
          this.tableColumnUser.forEach(column => {
    ...
    saveColumnWidth(columnKey, width) {
      ...
      localStorage.setItem('svn_user_column_widths', JSON.stringify(widths));
    },
    ...
    handleResetColumnWidth() {
      if (this.$refs.userTable) {
        this.$refs.userTable.resetColumnWidths();
      }
    },
    ...
    handleColumnVisibilityChange(visibleColumnKeys) {
      this.visibleTableColumns = this.tableColumnUser.filter(column => {
        const key = column.key || column.slot;
