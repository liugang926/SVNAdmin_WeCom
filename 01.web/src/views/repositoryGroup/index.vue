...
export default {
  ...
  methods: {
    ...
    loadColumnWidths() {
      // 从本地存储加载列宽设置
      const savedWidths = localStorage.getItem('svn_group_column_widths');
      if (savedWidths) {
        try {
          const widths = JSON.parse(savedWidths);
          this.tableGroupColumn.forEach(column => {
    ...
    saveColumnWidth(columnKey, width) {
      ...
      localStorage.setItem('svn_group_column_widths', JSON.stringify(widths));
    },
